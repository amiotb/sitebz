...
<svelte:head>
  <title>Projet {data.title} | Bérengère Amiot</title>
  <!-- TO DODO : hide html balises -->
  <meta
    name="description"
    content={`Projet ${data.title} : ${data.content.slice(0, 30)}`}
  />
</svelte:head>

<ImageZoom bind:show={show} bind:index={imageIndex} images={data.images} />
